...
void notifyLoad(unsigned int frame)
{
    if (currentAlgo == ALGO_FIFO) {
		...
		if (verbose)
			printf("[WSCLOCK] Frame %d loaded, timestamp=%d (hand at %d).\n", frame, lruTimestamp[frame], clockHand);
    }
}
...
void notifyAccess(unsigned int frame)
{
    if (currentAlgo == ALGO_LRU) {
		...
		if (vpn >= 0) {
			...
			if (verbose)
				printf("[WSCLOCK] Set ref bit R=1 for frame %d (VPN %d).\n",
					   frame, vpn);
		}
...
{   int victim = -1;
    ...
    /* -- CLOCK ---------------------------------- */
		...
		while (steps < NUM_FRAMES * 2) {
			int vpn = frameToVpn[clockHand];
			if (vpn >= 0) {
				int age = lruClockCounter - lruTimestamp[clockHand];
				// if (pageTable[vpn].referenced == 0 && age >= threshold) => evic the frame
				if (pageTable[vpn].referenced == 0 && age >= threshold) {
					if (verbose)
						printf("[WSCLOCK] Victim = frame %d (VPN %d, age=%d).\n", clockHand, vpn, age);
					break;
				} else {
					pageTable[vpn].referenced = 0;

					if (verbose)
						printf("[WSCLOCK] Frame %d (VPN %d) survives (R=%d, age=%d).\n", clockHand, vpn, pageTable[vpn].referenced, age);
				}
			}
...
void printReplacementInfo() {
	...
	case ALGO_CLOCK:
		...
		break;
	case ALGO_WSCLOCK:
		printf("WSCLOCK hand ==> frame %d\n", clockHand);
		printf("WSClock (Frame,Ref,Timestamp):\t");
