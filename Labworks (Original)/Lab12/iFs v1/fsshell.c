...
int cmd_ln(int argc, char* argv[])
{	char sourceName[MAXPATHNAME], targetName[MAXPATHNAME];
	int ret;
	...
	case 4:
		...
		if (strcmp(argv[1], "-s")==0) {
			 // source path may become an absolute path up to MAXPATHNAME; target is just a filename
			 if ((strlen(argv[2])>=MAXPATHNAME)||(strlen(argv[3])>MAXFILENAME)) {
				fprintf(stderr,"Sorry, SFS only support file name upto %d characters.\n", MAXFILENAME);
				return(-1);
			}
