...
// - when a eating philosopher is preempted, he will stop eating and proceed to put the chopsticks down
...
void eat(int i) {
	int j=0, time=3;
	while (state[i]==EATING && j<time) { // eat as long as noone preempts the chopsticks
		//if (j==0) printf("Philosopher %d is eating\n", i+1);
		sleep(1); j++;
	}
}
